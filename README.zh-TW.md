...
ErisPulse 是一個基於 Python 的事件驅動型多平台機器人開發框架。透過統一的 OneBot12 標準介面，您可以一次編寫程式碼，同時在雲湖、Telegram、OneBot 等多個平台部署相同功能的機器人。框架提供靈活的模組(`外掛`)系統、熱重載支援和完整的開發者工具鏈，適用於從簡單聊天機器人到複雜自動化系統的各種場景。

## 核心特性

- **事件驅動架構** - 基於 OneBot12 標準的清晰事件模型
- **跨平台相容** - 外掛模組編寫一次即可在所有平台使用
- **模組化設計** - 靈活的「外掛」系統，易於擴充和整合
- **熱重載支援** - 開發時無需重新啟動即可重新載入程式碼
...
> 如果您的 Python 版本低於 3.10，可以使用一鍵安裝腳本自動設定環境。詳見 [安裝腳本說明](scripts/install/)。
...
- **聊天助手** - 接入 AI 聊天模組，實現娛樂和互動
- **自動化工具** - 訊息通知、任務管理、資料收集
- **訊息轉發** - 跨平台訊息同步和轉發
...
- [沙箱](https://github.com/ErisPulse/ErisPulse-SandboxAdapter) - 網頁除錯介面，無需接實際平台

查看 [適配器詳細介紹](docs/platform-guide/README.md)
...
ErisPulse 專案的健全性還需要您的一份力！我們歡迎各種形式的貢獻，包括但不限於：

1. **回報問題**
   在 [GitHub Issues](https://github.com/ErisPulse/ErisPulse/issues) 提交錯誤報告

2. **功能請求**
   透過 [社群討論](https://github.com/ErisPulse/ErisPulse/discussions) 提出新想法

3. **程式碼貢獻**
   提交 Pull Request 前請閱讀我們的 [程式碼風格](docs/styleguide/) 以及 [貢獻指南](CONTRIBUTING.md)
