...
class SelfUpdateCommand(Command):
    ...
    def add_arguments(self, parser: ArgumentParser):
        parser.add_argument(
            "version", nargs="?", help="要更新到的版本号 (可选，默认为最新版本)"
        )
    ...
    def _select_target_version(
        ...
        if latest_stable:
            console.print(
                Text(
                    f"    1.  最新稳定版 ({latest_stable['version']})", style="success"
                )
            )
        ...
        if include_pre and latest_pre:
            ...
            console.print(
                Text(
                    f"    {idx}.  最新预发布版 ({latest_pre['version']})",
                    style="warning",
                )
            )
    ...
    def _select_from_version_list(self, versions, include_pre: bool = False) -> str:
        ...
        while True:
            ...
            for i, v in enumerate(batch):
                vtype = (
                    "[warning]预发布[/]" if v["pre_release"] else "[success]稳定版[/]"
                )
                table.add_row(
                    str(page_start + i + 1),
                    v["version"],
