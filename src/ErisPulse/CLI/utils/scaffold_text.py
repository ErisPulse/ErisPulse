...
_EN_FALLBACK: dict[str, str] = {
    ...
    "cfg.header.usage": "# To use: copy the items you need into config.toml and modify as needed",
    "cfg.section.server": "# ==================== Server ====================",
    "cfg.section.logger": "# ==================== Logging ====================",
    "cfg.section.storage": "# ==================== Storage ====================",
    "cfg.section.event": "# ==================== Events ====================",
    "cfg.section.framework": "# ==================== Framework ====================",
    "cfg.section.router": "# ==================== Router ====================",
    "cfg.section.adapter_status": "# ==================== Adapter Status ====================",
    "cfg.section.module_status": "# ==================== Module Status ====================",
    "cfg.section.components": "# ==================== Installed Components (declarative, auto-generated) ====================",
    "cfg.section.components_hint": "# Rendered from each installed adapter/module ConfigClass; example: fields only live here (not auto-persisted to config.toml)",
    "cfg.server.host": "Listen address",
    "cfg.server.port": "Listen port",
    "cfg.server.auto_start": "Auto start HTTP server (set false for pure WS/polling adapters)",
    "cfg.server.ssl_certfile": "SSL cert file path (empty = disabled)",
    "cfg.server.ssl_keyfile": "SSL key file path (empty = disabled)",
    "cfg.server.ssl_inline_hint": "# Default certs live in config/ssl/ (relative paths follow the project dir). "
    "Alternatively use inline PEM below (takes priority over paths):",
    "cfg.logger.level": "Log level: DEBUG/INFO/WARNING/ERROR",
    "cfg.logger.log_files": "Log file list, e.g. [\"logs/app.log\"] (mutually exclusive with log_dir, higher priority)",
    "cfg.logger.log_dir": "Log directory, e.g. \"logs\" (enables auto rotation)",
    "cfg.logger.log_rotation": "Rotation mode: \"size\" / \"date\" / \"none\"",
    "cfg.logger.log_max_size_mb": "size mode: max file size (MB)",
    "cfg.logger.log_backup_count": "Number of rotated log files to keep",
    "cfg.logger.log_rotation_when": "date mode: rotation period S/M/H/D/midnight",
    "cfg.logger.memory_limit": "In-memory log entries limit",
    "cfg.storage.use_global_db": "Use global database",
    "cfg.storage.backend": "Storage backend: \"sqlite\" / \"mysql\" / \"postgres\"",
    "cfg.storage.mysql": "MySQL connection params (effective when backend = \"mysql\"), requires: pip install ErisPulse[mysql]",
    "cfg.storage.postgres": "PostgreSQL connection params (effective when backend = \"postgres\"), requires: pip install ErisPulse[postgres]",
    "cfg.event.ignore_self": "Ignore self messages",
    "cfg.command.prefix": "Command prefix",
    "cfg.command.case_sensitive": "Case sensitive",
    "cfg.command.allow_space_prefix": "Allow spaces before prefix",
    "cfg.command.must_at_bot": "Must @bot to trigger",
    "cfg.framework.enable_lazy_loading": "Enable module lazy loading (default true)",
    "cfg.framework.plugins_dir": "Local plugins folder (relative to project root, string or list)",
    "cfg.framework.uninit_timeout": "Unified uninit/graceful-shutdown timeout (secs), covers:",
    "cfg.framework.uninit_timeout_line1": "#   - per-module on_unload / per-adapter shutdown force-kill timeout",
    "cfg.framework.uninit_timeout_line2": "#   - uninit() overall flow timeout",
    "cfg.framework.strict_mode": "Strict mode (reject loading modules/adapters not whitelisted)",
    "cfg.framework.strict_mode_exceptions": "Modules/adapters exempted in strict mode",
    "cfg.framework.handler_max_concurrency": "Max concurrent event handlers",
    "cfg.framework.proactive_gc_interval": "Proactive GC interval (secs, 0 disables)",
    "cfg.framework.proactive_gc_generation": "Regular collection generation (0/1/2)",
    "cfg.framework.proactive_gc_full_every": "Full collection every N rounds (0 disables)",
    "cfg.framework.proactive_gc_memory_growth_mb": "Full collection memory growth threshold (0 = unlimited)",
    "cfg.framework.proactive_gc_idle_only": "Skip Python GC during event bursts",
    "cfg.framework.proactive_gc_gen0_min": "gen0 garbage minimum, skip if below",
    "cfg.framework.offline_bot_expiry": "Offline bot expiry cleanup (secs)",
}
...
_TRANSLATIONS: dict[str, dict[str, str]] = {
    ...
    "cfg.section.server": "# ==================== 服务器 ====================",
    "cfg.section.logger": "# ==================== 日志 ====================",
    "cfg.section.storage": "# ==================== 存储 ====================",
    "cfg.section.event": "# ==================== 事件系统 ====================",
    "cfg.section.framework": "# ==================== 框架 ====================",
    "cfg.section.router": "# ==================== 路由增强 ====================",
    "cfg.section.adapter_status": "# ==================== 适配器状态 ====================",
    "cfg.section.module_status": "# ==================== 模块状态 ====================",
    "cfg.section.components": "# ==================== 已安装组件（声明式自动生成） ====================",
    "cfg.section.components_hint": "# 由各已安装适配器/模块的 ConfigClass 渲染；example: 字段仅记录于此（不会自动写入 config.toml）",
    "cfg.server.host": "监听地址",
    "cfg.server.port": "监听端口",
    "cfg.server.auto_start": "是否自动启动 HTTP 服务器 (纯 WS/轮询适配器可设 false)",
    "cfg.server.ssl_certfile": "SSL 证书路径（留空表示不启用）",
    "cfg.server.ssl_keyfile": "SSL 密钥路径（留空表示不启用）",
    "cfg.server.ssl_inline_hint": "# 默认证书放 config/ssl/ 目录（相对路径跟随项目运行目录）；也可用下方内联 PEM（优先于路径）：",
    "cfg.logger.level": "日志级别: DEBUG/INFO/WARNING/ERROR",
    "cfg.logger.log_files": "日志文件列表, 如 [\"logs/app.log\"]（与 log_dir 互斥，优先级更高）",
    "cfg.logger.log_dir": "日志目录, 如 \"logs\"（设置后自动分段轮转）",
    "cfg.logger.log_rotation": "分段方式: \"size\"(按大小) / \"date\"(按时间) / \"none\"",
    "cfg.logger.log_max_size_mb": "size 模式单文件上限（MB）",
    "cfg.logger.log_backup_count": "保留的历史日志文件数",
    "cfg.logger.log_rotation_when": "date 模式轮转周期: S/M/H/D/midnight",
    "cfg.logger.memory_limit": "内存日志条数上限",
    "cfg.storage.use_global_db": "是否使用全局数据库",
    "cfg.storage.backend": "存储后端: \"sqlite\" / \"mysql\" / \"postgres\"",
    "cfg.storage.mysql": "MySQL 连接参数（backend = \"mysql\" 时生效），需安装: pip install ErisPulse[mysql]",
    "cfg.storage.postgres": "PostgreSQL 连接参数（backend = \"postgres\" 时生效），需安装: pip install ErisPulse[postgres]",
    "cfg.event.ignore_self": "忽略自身消息",
    "cfg.command.prefix": "命令前缀",
    "cfg.command.case_sensitive": "区分大小写",
    "cfg.command.allow_space_prefix": "允许前缀前有空格",
    "cfg.command.must_at_bot": "必须艾特Bot才触发",
    "cfg.framework.enable_lazy_loading": "启用模块懒加载（默认 true）",
    "cfg.framework.plugins_dir": "本地插件文件夹（相对项目根目录，支持字符串或列表）",
    "cfg.framework.uninit_timeout": "反初始化/优雅收尾统一超时（秒），同时约束：",
    "cfg.framework.uninit_timeout_line1": "#   - 单个模块 on_unload / 单个适配器 shutdown 超时强杀",
    "cfg.framework.uninit_timeout_line2": "#   - uninit() 整体流程兜底超时",
    "cfg.framework.strict_mode": "严格模式（模块/适配器白名单外的拒绝加载）",
    "cfg.framework.strict_mode_exceptions": "严格模式下豁免的模块/适配器",
    "cfg.framework.handler_max_concurrency": "事件处理器同时并发数上限",
    "cfg.framework.proactive_gc_interval": "主动 GC 间隔（秒，0 禁用）",
    "cfg.framework.proactive_gc_generation": "常规回收分代 (0/1/2)",
    "cfg.framework.proactive_gc_full_every": "每 N 轮一次全量回收（0 禁用）",
    "cfg.framework.proactive_gc_memory_growth_mb": "全量回收的内存增长门限（0 不设限）",
    "cfg.framework.proactive_gc_idle_only": "事件洪峰时跳过 Python GC",
    "cfg.framework.proactive_gc_gen0_min": "gen0 垃圾量下限，低于则跳过回收",
    "cfg.framework.offline_bot_expiry": "离线 Bot 过期清理（秒）",
    },
    "zh-TW": {
