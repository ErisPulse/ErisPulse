...
# Conversation 对话原语 / 消息事务 / 检查点 GC 已拆分至独立模块，
# 此处 re-export 保持既有导入路径不变
from .conversation import (  # re-export：拆分自本模块，保持既有导入路径不变
    Conversation,
    _conversation_resume_handlers,  # noqa: F401 （tests 直接从 wrapper 引用）
    _MessageTx,
    _rollback_receipts,  # noqa: F401 （tests 直接从 wrapper 引用）
)
