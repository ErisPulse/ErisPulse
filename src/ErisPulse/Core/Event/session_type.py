...
def register_custom_type(
    ...
    try:
        ...
        logger.debug(f"已注册自定义会话类型: {receive_type_key} → {send_type} (ID字段: {id_field})")
        return True
...
def unregister_custom_type(receive_type: str, platform: Optional[str] = None) -> bool:
    """
    注销自定义会话类型
...
def get_id_field(receive_type: str, platform: Optional[str] = None) -> str:
    """
    根据接收类型获取对应的ID字段名
...
def get_receive_type(id_field: str, platform: Optional[str] = None) -> str:
    """
    根据ID字段获取对应的接收类型
...
def convert_to_send_type(receive_type: str, platform: Optional[str] = None) -> str:
    """
    将接收类型转换为发送目标类型
...
def convert_to_receive_type(send_type: str, platform: Optional[str] = None) -> str:
    """
    将发送目标类型转换为接收类型
...
def infer_receive_type(event: Dict, platform: Optional[str] = None) -> str:
    """
    根据事件数据自动推断接收类型
...
def get_target_id(event: Dict, platform: Optional[str] = None) -> str:
    """
    获取事件的目标ID（根据推断的会话类型）
...
def clear_custom_types(platform: Optional[str] = None) -> int:
    """
    清除自定义类型映射
    ...
    if platform:
        # 只清除指定平台的类型
        to_remove = [k for k in _custom_type_to_id_field if k.startswith(f"{platform}_")]
        count = len(to_remove)
