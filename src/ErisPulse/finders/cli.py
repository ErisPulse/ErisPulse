"""
ErisPulse CLI扩展发现器

专门用于发现和查找 ErisPulse CLI 扩展的 entry-points

{!--< tips >!--}
1. 查找 erispulse.cli 组的 entry-points
2. 支持缓存机制，避免重复查询
3. 提供便捷的查询接口
{!--< /tips >!--}
"""

from typing import List, Optional, Any, Dict
from .bases.finder import BaseFinder


class CLIFinder(BaseFinder):
    """
    CLI扩展发现器
    
    负责发现 ErisPulse CLI 扩展的 entry-points
    
    {!--< tips >!--}
    使用方式：
    >>> finder = CLIFinder()
    >>> # 查找所有CLI扩展
    >>> cli_extensions = finder.find_all()
    >>> # 按名称查找
    >>> extension = finder.find_by_name("my_extension")
    >>> # 获取CLI扩展映射
    >>> extension_map = finder.get_entry_point_map()
    >>> # 检查CLI扩展是否存在
    >>> if "my_extension" in finder:
    ...     print("CLI扩展存在")
    {!--< /tips >!--}
    """
    
    def _get_entry_point_group(self) -> str:
        """
        获取 entry-point 组名
        
        :return: "erispulse.cli"
        """
        return "erispulse.cli"
    
    def get_all_names(self) -> List[str]:
        """
        获取所有CLI扩展名称
        
        :return: CLI扩展名称列表
        """
        return list(self.get_entry_point_map().keys())
    
    def get_all_packages(self) -> List[str]:
        """
        获取所有CLI扩展所属的 PyPI 包名
        
        :return: PyPI 包名列表
        """
        packages = set()
        for entry in self.find_all():
            if hasattr(entry, 'dist') and entry.dist:
                packages.add(entry.dist.name)
        return list(packages)
    
    def get_package_for_extension(self, extension_name: str) -> Optional[str]:
        """
        获取指定CLI扩展所属的 PyPI 包名
        
        :param extension_name: CLI扩展名称
        :return: PyPI 包名，未找到返回 None
        """
        entry = self.find_by_name(extension_name)
        if entry and hasattr(entry, 'dist') and entry.dist:
            return entry.dist.name
        return None
    
    def get_extension_info(self, extension_name: str) -> Optional[Dict[str, Any]]:
        """
        获取CLI扩展的完整信息
        
        :param extension_name: CLI扩展名称
        :return: CLI扩展信息字典，未找到返回 None
        
        :return:
            Dict: {
                "name": CLI扩展名称,
                "package": PyPI 包名,
                "version": 版本号,
                "entry_point": entry-point 对象
            }
        """
        entry = self.find_by_name(extension_name)
        if not entry:
            return None
        
        info = {
            "name": entry.name,
            "entry_point": entry
        }
        
        if hasattr(entry, 'dist') and entry.dist:
            info["package"] = entry.dist.name
            info["version"] = entry.dist.version
        
        return info
    
    def get_extensions_by_package(self, package_name: str) -> List[str]:
        """
        获取指定 PyPI 包下的所有CLI扩展名称
        
        :param package_name: PyPI 包名
        :return: CLI扩展名称列表
        """
        extensions = []
        for entry in self.find_all():
            if hasattr(entry, 'dist') and entry.dist:
                if entry.dist.name == package_name:
                    extensions.append(entry.name)
        return extensions
