...
- @YingXinche
  ...
  - `Core/Event/command` 修复空格分隔的子命令名（如 `admin add`）注册后永远无法触发的问题（可入帮助列表但分发仅匹配首 token）：命令匹配改为最长前缀匹配，子命令名之后的 token 作为参数，作用域/ACL/权限链对命令全名生效；子命令未声明 `permission` 时继承父链上最近声明权限的祖先命令，仅注册父命令时行为不变
