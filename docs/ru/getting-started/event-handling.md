...
@command("help", help="Отображает информацию о помощи")
...
```python
@command(["help", "h"], aliases=["帮助"], help="Отображает информацию о помощи")
async def help_handler(event):
    await event.reply("Информация о помощи...")
```

Пользователь может вызывать команду следующими способами:
- `/help`
- `/h`
- `/帮助`
...
async def echo_handler(event):
    ...
    if not args:
        await event.reply("Введите сообщение для эхо")
    else:
...
Параметр `group` используется только для группировки в списке помощи; в приведённом примере `admin.reload` — это **целое имя команды** (точка используется только для стиля, пользователь должен ввести `/admin.reload`).

### Подкоманды

Имена команд могут содержать **множественные токены**, разделённые пробелами, реализуя подкоманды вроде `/admin add` или `/admin user ban`:
...
Правила сопоставления (сопоставление по **длиннейшему префиксу**):

- `/admin add x` сначала попадает в `admin add`, `event.get_command_args()` возвращает `["x"]` (параметры после имени подкоманды)
- Если зарегистрирована только команда `admin`, то `/admin add x` попадает в `admin`, `get_command_args()` возвращает `["add", "x"]` (обычное поведение)
- Алиасы поддерживают множественные токены (например, `a remove`), а также одиночные токены (например, `a`) для указания на подкоманды
- Если зарегистрированы родительская и подкоманды, то ввод несуществующей подкоманды (например, `/admin list x`) возвращается к родительской команде

**Наследование прав**: если подкоманда не объявляет `permission`, она автоматически наследует права ближайшей родительской команды, которая их объявила — защита `/admin` автоматически защищает все его подкоманды; права, объявленные в самой подкоманде, имеют приоритет:
...
# permission не нужно объявлять повторно, наследуется is_admin
@command("admin add", help="Добавить администратора")
async def admin_add_handler(event):
    ...
```

Примечание: `master=True` и `hidden` **не** наследуются, при необходимости их нужно объявлять отдельно в подкоманде; пользовательские ACL (списки разрешений/запретов) соответствуют полному имени команды, шаблоны glob (например, `"admin*") могут охватывать всю группу подкоманд.
...
```python
# ① ACL команды (конфигурация со стороны пользователя): по белому и чёрному спискам пользователей для команды, при отклонении возвращается "Недостаточно прав"
# ② master=True — доступ только для владельца фреймворка (фреймворк автоматически проверяет, при отклонении возвращается "Недостаточно прав")
@command("restart", master=True, help="Перезапустить модуль")
async def restart_handler(event):
    await event.reply("Модуль перезапущен")

# ③ permission=вызываемая функция — логика контроля команды (выполняется, если функция возвращает True)
def is_admin(event):
...
**ACL пользователей команд** (`ErisPulse.event.command.acl`): пользователь может настроить белый и чёрный списки для любой команды, имена команд поддерживают точные и шаблоны glob (например, `"roll*"), при отклонении возвращается "Недостаточно прав":

```toml
# config.toml — разрешить только 123456 выполнение restart; 666 всегда запрещён
[ErisPulse.event.command.acl.restart]
allow = ["onebot11:123456"]
deny = ["onebot11:666"]
```

Порядок проверки: если命中 `deny` → отклонение; если `allow` не пуст и не命中 → отклонение; если ACL не настроена, используется `event.command.default_allow` (`false` = строгий режим, без ACL — отклонение; `true` — передаётся разработчику, `master=True` / `permission`). API во время выполнения (поддержка шаблонов glob для имени команды):
...
command.remove_acl("restart")                          # Очистить списки разрешений/запретов
...
Контроль доступа на уровне событий (кто может получать сообщения от кого / от какой группы / от какого бота) реализуется через **идентификационные области действия** (`scope.identity`); доступность модуля на уровне модуля (какие модули можно использовать) реализуется через **области действия платформы/ботов/сессий** (`scope.platforms / bots / sessions`).
См. [Области действия (scope)](../advanced/scope.md).

> Рекомендуется: использовать `master=True` / `permission` для внутренней бизнес-логики; использовать идентификационные области для контроля доступа по пользователю / группе; использовать области действия модуля для контроля доступности модуля.
...
Система событий ErisPulse использует модель **параллельной обработки в рамках одного приоритета и последовательной обработки между разными приоритетами**:
...
- **Параллельная обработка одного приоритета**: обработчики с одинаковым приоритетом выполняются одновременно, повышая пропускную способность
- **Последовательная обработка разных приоритетов**: группы с разными приоритетами выполняются последовательно (чем выше приоритет, тем раньше он выполняется), обеспечивая выполнение обработчиков высокого приоритета первыми
- **Copy-On-Write**: обработчики не создают копию, если не изменяют данные, обеспечивая нулевой накладной расход
- **Обработка конфликтов**: при одновременном изменении одного и того же поля несколькими обработчиками одного приоритета используется последнее значение, и в лог записывается предупреждение
- **Механизм прерывания**: при вызове `event.done()` (по умолчанию) или `event.done(claim=False)` любым обработчиком, последующие группы с более низким приоритетом пропускаются. Разница между "принятием" и "блокировкой" описана в разделе [Контроль цепочки: принятие и блокировка](#контроль-цепочки-принятие-и-блокировка)
...
# Последовательное выполнение разных приоритетов
@message.on_message(priority=10)
...
> **Ограничение параллелизма**: все совпадающие обработчики немедленно создаются в виде задач, но ограничиваются сигналом, ограничивающим **максимальное количество одновременно выполняемых задач** по умолчанию **64** (`ErisPulse.framework.handler_max_concurrency`, поддерживается горячая настройка). Задачи, превышающие лимит, ждут в очереди на сигнале, пока предыдущие не завершатся. В момент пикового количества событий это ваш "редуктор давления".
>
> **Медленные логи**: если один обработчик выполняется дольше **1 секунды**, фреймворк записывает в лог предупреждение (`handler_slow`). Время ожидания `wait_reply` исключается из времени выполнения, чтобы не ошибочно помечать "медленными" ожидание ответа.
