...
После поступления события есть два **тихих** фильтра (никаких ответов и ошибок):

1. **Область идентификации** (`ErisPulse.scope.identity`): при поступлении события в точку распределения оно проверяется по пользователю > группе > боту > адаптеру, и если не проходит, **всё событие** отбрасывается, и ни один обработчик (включая обработчик команд) не срабатывает.
2. **Область модуля** (`ErisPulse.scope`): когда событие достигает обработчика или команды модуля, проверяется, доступен ли модуль, по сессии > боту > платформе, **если не проходит, silently пропускается**.
...
При этом сообщения из этой группы не будут доставляться обработчикам команд и событий `MyModule`. Это не ошибка, а механизм фильтрации — при диагностике "модуль не реагирует" сначала проверьте привязку областей идентификации и модуля.
...
- Обработчики на уровне фреймворка (например, обработчик команд `scope_exempt=True`) не подвержены влиянию **области модуля**, но подвержены влиянию **области идентификации** (всё событие уже отброшено)
- Перед выполнением команды есть третий фильтр: ACL пользователей команды (при отказе возвращается "Недостаточно прав", см. предыдущий раздел)
- Четвертый фильтр — **переопределение событий** (см. следующий раздел)

> Настройки области, синтаксис сопоставления, API во время выполнения см. в [Области (scope)](../../advanced/scope.md).
...
При регистрации обработчиков событий параметры, объявленные в декораторах (pattern / regex / master / hidden и т.д.), являются **по умолчанию для разработчика**. Система общего переопределения позволяет пользователю переопределить поведение любого модуля по типу события — стандартные OneBot12 типы (meta / message / notice / request) и расширенные типы ErisPulse (command) имеют свои собственные переопределяемые параметры:
...
```toml
# message: переопределение условий триггера текста (и условия в коде AND)
...
# command: переопределение параметров реализации (приоритет пользователя — можно ужесточить или ослабить настройки разработчика)
[ErisPulse.event.overrides.command.MyModule.restart]
master = true
hidden = true

# acl: белый и черный списки пользователей команды (по шаблону имени команды)
[ErisPulse.event.overrides.acl."roll*"]
allow = ["onebot11:u_vip"]

# ACL по умолчанию (false = строгий режим: без ACL — отказ)
acl_default_allow = true
```

API во время выполнения (`from ErisPulse.Core.Event import overrides` или `sdk.Event.overrides`, **типы подпространств имен** — симметричные `set` / `get` / `delete` для каждого типа):
...
- Условия переопределения и условия в обработчиках кода **одновременно действуют** (логическое И); параметры `command` и объявленные разработчиком **глубоко объединяются** (приоритет переопределения)
- `detail_types`: события без `detail_type` разрешаются (не убивают неизвестные события)
- `pattern` / `regex`: события без текста (connect / heartbeat и т.д.) не подвергаются ограничению, сразу разрешаются
- `command` переопределение ключа `master` синхронно отображается в хранилище ключа `must_master`; запрет команды идет через `acl` deny
- Настройки изменяются немедленно (горячее обновление), проверка формата предупреждает (неизвестные параметры / плохие записи игнорируются)
...
Метод `event.reply()` поддерживает различные параметры для отправки сообщений с упоминанием, ответом и т.д.:
...
# Ответ на сообщение
await event.reply("Ответ на сообщение", reply_to="msg_id")
...
Ожидание подтверждения или отрицания, автоматическое распознавание встроенных слов подтверждения на китайском и английском:
...
**Режим слияния**: `merge_prompt=True` вставляет опции в текст сообщения, отправляя все одним сообщением с указанным `method`:
...
> Заполнитель `{options}` контролирует позицию вставки опций; если не указан, опции добавляются в конец prompt. Можно изменить заполнитель с помощью параметра `placeholder` (например, `placeholder="[choices]"`). `options_format="auto"` (по умолчанию) автоматически выбирает стиль в зависимости от метода: Markdown → маркированный список, Html → нумерованный список, другие → текстовый список. Для текстовых методов (Text/Markdown/Html и т.д.) опции по умолчанию объединяются в конец; для не-текстовых методов (Image и т.д.) по умолчанию опции разделяются на две отдельные сообщения.
...
Ожидание события, соответствующего заданным условиям, без ограничения на пользователя:
...
Создание интерактивного многоэтапного контекста диалога:
