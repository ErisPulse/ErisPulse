...
# Не рекомендуется использовать aiohttp напрямую (неудобно для управления фреймворком)
import aiohttp
...
```python
from ErisPulse.Core.Event import Event  # Подсказки IDE доступны благодаря аннотации event: Event

async def handle_command(self, event: Event):
    # Длительные операции, результат которых необходимо ждать: просто await (жизненный цикл ясен)
    result = await self._long_operation()

async def on_load(self, event: dict):
    # Фоновые задачи (опрос/таймер/fire-and-forget): используйте self.spawn(),
    # при выгрузке модуля фреймворк отменяет задачи после on_unload, избегая утечек ссылок на self
    self.spawn(self._poll())
```

> [!NOTE]
> Рекомендуется использовать `self.spawn()` (ErisPulse **2.8.0+**). С версии **2.8.3** даже прямое использование `asyncio.create_task`
> автоматически привязывается к модулю (Task Factory автоматически регистрируется, задачи отменяются при выгрузке модуля, утечек `self` больше нет);
> `self.spawn()` по-прежнему рекомендуется — поддерживает переключение потоков и явное указание `owner=`.
> В версиях до **2.8.3** прямые задачи не привязываются к модулю и удерживают ссылку на `self`, что приводит к утечке модуля
> (утечка при горячей перезагрузке), поэтому необходимо использовать `self.spawn()`. Подробнее см. [Управление жизненным циклом](../../advanced/lifecycle.md#фоновые-задачи-привязка-и-автоматическая-отмена).
...
async def on_unload(self, event):
    # Если нужно использовать кастомный клиент, не забудьте очистить ресурсы
    pass
```
