...
## Связь с декларативным конфигурационным классом

Объявление модели и `ConfigClass` (`@dataclass + field(metadata=...)`) **используют один и тот же底层** — словарь ограничений, движок проверки (`validate_field_constraints`), таблица регистрации типов (`python_type_category`); но базовые классы намеренно разделены: поля конфигурации — обычные значения (TOML-перевод, горячая перезагрузка), поля моделей — описатели столбцов (доступ к классу = выражение запроса, построчная обработка экземпляра). Один синтаксис объявления, две разные базы.
