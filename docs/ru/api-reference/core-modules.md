...
Управление ожиданием ответа (wait_reply) и взаимоисключающими сеансами (sdk.interaction).

### Общие методы

```python
# Получить текущего владельца сеанса (кто взаимодействует с пользователем)
owner = sdk.interaction.get_owner_of(event)

# Захватить взаимоисключающий сеанс (возвращает None, если занят)
...
# Статистика ожидания сеанса
sdk.interaction.counts()  # {'waits': 2, 'leases': 1, 'owners': {'Chat': 3}}
```

При отключении модуля / остановке адаптера все ожидания автоматически отменяются (ожидающие немедленно получают `None`), а при совпадении ответа автоматически проверяется права scope (если пользователь заблокирован / модуль откреплен, ожидание прекращается).

> Добавлено начиная с 2.8.0-dev.2

## Ящик входящих сообщений Transcript

Автоматическая запись и запрос последних сообщений в рамках сессии (`sdk.transcript`) служит базовым компонентом для модулей, использующих контекстную память, таких как ИИ-диалоги и предотвращение повторного ввода.
...
Конфигурация (`ErisPulse.transcript`): `enabled` (включено по умолчанию), `max_per_session` (максимум сообщений в сессии, по умолчанию 50), `ttl_hours` (время жизни сообщений, по умолчанию 168 часов). Данные хранятся в отдельной таблице SQLite, избыток или просроченные данные удаляются лениво при необходимости.
