...
|------|------|------|------|
| `config` | `cfg`, `conf` | `[name] [--list/-l]` | Интерактивная конфигурация декларативных параметров адаптера/модуля |

### config

Интерактивное заполнение декларативных параметров конфигурации адаптера/модуля. Мастер запускается на основе класса конфигурации, объявленного адаптером/модулем (`ConfigClass` / `AccountConfigClass`), автоматически генерирует форму и проверяет ввод, без необходимости ручного написания `config.toml`.

Адаптер дополнительно поддерживает управление несколькими аккаунтами (бот аккаунтами): добавление/редактирование/удаление аккаунтов, а также включение/отключение переключателей.
...
| Параметр | Краткий параметр | Описание |
|------|--------|------|
| `[name]` | | Имя цели (имя платформы адаптера или имя модуля), оставьте пустым для перехода в интерактивный выбор |
| `--list` | `-l` | Вывести только список состояний конфигурации всех целей, не запуская мастер |

**Примеры:**

```bash
# Просмотр состояния конфигурации всех адаптеров/модулей
epsdk config --list

# Интерактивный выбор цели для конфигурации
epsdk config

# Прямая конфигурация указанного адаптера
epsdk config yunhu

# Прямая конфигурация указанного модуля
epsdk config MyModule
```

**Описание:**

- Состояние конфигурации делится на четыре категории: `Готово` (проверка пройдена), `Незавершено` (отсутствуют обязательные поля или проверка не пройдена), `Не настроено` (никогда не генерировалось), `Без конфигурации` (цель не объявляет класс конфигурации)
- Значения полей снабжены пометкой источника: при наличии конфигурации отображается `（Текущее:значение）`, при отсутствии конфигурации отображается значение по умолчанию схемы `（По умолчанию:значение）`; нажатие Enter сохраняет текущее значение
- В полях типа "ключ" (объявленных как `secret`) ввод не отображается, нажатие Enter сохраняет уже установленное значение
- В режиме интерактивного выбора, после завершения одного мастера возвращается в меню выбора (состояние обновлено), можно последовательно настроить несколько целей, оставив пустым для выхода
- При неудачной проверке всей формы и отказе от повторного заполнения, текущий мастер завершается и никакая конфигурация не записывается (избегается состояние "включено, но конфигурация незавершена")
- После сохранения конфигурация немедленно записывается в `config/config.toml`, доступна в Dashboard и запущенном SDK; для применения новой конфигурации аккаунта запущенному адаптеру достаточно перезапустить процесс
- После успешной установки адаптера с помощью `epsdk install` (интерактивная установка) или `epsdk init`, если обнаружено объявление конфигурации, автоматически запускается этот мастер; при прямой установке пакета через командную строку выводится только подсказка о конфигурации
...
# Интерактивный выбор целевой конфигурации
epsdk config
...
# Только генерация stub-файлов модуля
