...
1. [SDK 入口对象](#sdk-入口对象)
2. [初始化方法 `sdk.init()`](#初始化方法-sdkinit)
3. [日志记录器 (Logger)](#日志记录器-logger)
4. [环境配置管理 (Env)](#环境配置管理-env)
5. [错误处理 (Raiserr)](#错误处理-raiserr)
6. [工具函数 (Util)](#工具函数-util)
7. [模块系统 (Mods)](#模块系统-mods)
8. [适配器系统 (Adapter)](#适配器系统-adapter)
   - [BaseAdapter 接口规范](#baseadapter-接口规范)
9. [DSL 风格消息发送接口（SendDSL）](#dsl-风格消息发送接口-senddsl)
   - [定义消息类型](#定义消息类型)
   - [必须实现的方法：Text(...)](#必须实现的方法-text)
   - [可选推荐方法](#可选推荐方法)
   - [内部工作原理简析](#内部工作原理简析)
   - [自定义扩展示例：Card(...) 消息](#自定义扩展示例-card--消息)
   - [开发者建议](#开发者建议)
10. [模块开发基础](#模块开发基础)
    - [目录结构](#目录结构)
...
> 更多详细信息请阅读源码或参考[ErisPulse 开发者文档](docs/DEVELOPMENT.md)。