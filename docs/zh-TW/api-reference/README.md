...
| [適配器系統 API](adapter-system.md) | Adapter 管理器、SendDSL、中介軟體、Bot 狀態管理的 API 參考 |
| [自動產生 API](auto_api/README.md) | 從原始碼 docstring 自动生成的完整 API 文件 |

> 手動編寫的 API 文件側重於使用範例和快速查詢；自動產生的 API 文件包含完整的類別/方法簽名，兩者互補。
...
| 模組 | 訪問路徑 | 說明 |
|------|---------|------|
| `sdk.storage` | `sdk.storage` | 基於 SQLite 的鍵值儲存 + SQL 串接查詢 |
...
| `notice` | `ErisPulse.Core.Event.notice` | 通知事件（好友、群成員變更） |
...
- [進階主題](../advanced/) - 路由、HTTP 客戶端、SQL 建構器等深入文件