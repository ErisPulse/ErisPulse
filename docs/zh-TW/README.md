...
| 文件 | 說明 |
|------|------|
| [架構概覽](docs/zh-TW/architecture.md) | 可視化圖表：核心架構、初始化流程、事件處理、生命週期、模組加載策略（含 `activate_on` 事件驅動懶激活）、本地插件資料夾與模組熱重載架構（支援全部模組來源） |
...
| [互動會話系統](docs/zh-TW/advanced/interaction.md) | wait_reply 全解、會話定時器（remind/escalate）、多路等待（select）、會話互斥租約、收件箱、訊息事務、鏈路追蹤 |
...
| [ErisPulse-Dashboard](docs/zh-TW/ecosystem/dashboard.md) | Web 管理面板 + 視窗註冊 API（模組可向側邊欄註冊自訂頁面） |
