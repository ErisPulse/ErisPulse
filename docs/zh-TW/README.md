...
| **測試與排查** | [模組測試（ErisPulse-Testing）](docs/zh-TW/developer-guide/testing.md) | 分發決策鏈排查「命令為什麼沒觸發」 |
...
| 文件 | 說明 |
|------|------|
| [架構概覽](docs/zh-TW/architecture.md) | 可視化圖表：核心架構、初始化流程、事件處理、生命週期、模組加載策略（含 `activate_on` 事件驅動懶加載）、本地插件文件夾與模組熱重載架構（支援全部模組來源） |
...
| [作用域（scope）](docs/zh-TW/advanced/scope.md) | 三維作用域控制：模組可用性 / 事件准入 / 出站動作限制（含方法級細粒度規則、綁定繼承 merge） |
| [歸屬權（owner）系統](docs/zh-TW/advanced/ownership.md) | 資源歸屬與自動回收：owner 上下文、歸屬資源全景、卸載清理序列、設計邊界與模組作者指南 |
| [影子模組與灰度轉正](docs/zh-TW/advanced/shadow.md) | 模組新版本以獨立 owner 並行試運行：五道隔離閘、行為 diff 對齊、promote 轉正與失敗回滾 |
| [互動會話系統](docs/zh-TW/advanced/interaction.md) | wait_reply 全解、會話定時器（remind/escalate）、多路等待（select）、會話互斥租約、收件箱、消息事務、鏈路追蹤 |
| [模組間通信](docs/zh-TW/advanced/module-communication.md) | RPC 協議化（module.call）、meta.services 服務契約與目錄、定向事件 emit(to=)、冷啟動回放、事件冪等去重 |
| [Conversation 多輪對話](docs/zh-TW/advanced/conversation.md) · [MessageBuilder](docs/zh-TW/advanced/message-builder.md) · [SQL 構建](docs/zh-TW/advanced/sql-builder.md) · [存儲後端](docs/zh-TW/advanced/storage-backends.md) · [HTTP 客戶端](docs/zh-TW/advanced/http-client.md) · [國際化](docs/zh-TW/advanced/i18n.md) | 進階工具 |
...
| [ErisPulse-Dashboard](docs/zh-TW/ecosystem/dashboard.md) | Web 管理介面 + 視窗註冊 API（模組可向側邊欄註冊自定義頁面） |
| [ErisPulse-Cron](docs/zh-TW/ecosystem/cron.md) | 定時任務調度模組：一次性 / 间隔 / Cron 表達式，回調傳參，SQLite 持久化，其他模組可托管定時回調 |
