...
| 文件 | 說明 |
|------|------|
| [架構概覽](docs/zh-TW/architecture.md) | 可視化圖表：核心架構、初始化流程、事件處理、生命週期、模組加載策略（含 `activate_on` 事件驅動懶加載）、本地插件資料夾與模組熱重載架構（支援全部模組來源） |
...
| [ErisPulse-Takumi](docs/zh-TW/ecosystem/takumi.md) | 圖片渲染（HTML / 節點樹 / SVG / 動畫，內建中英文字體） |
