...
> 本指南是 [5 分鐘快速開始](../quick-start.md) 的**深入補充**。如果你還沒有成功啟動第一個機器人，請先完成快速開始。

機器人啟動後，這裡將帶你系統地理解框架的核心概念和常用功能。
...
- **適配器系統** — 與各平台通信，將平台事件轉換為統一的 OneBot12 標準格式
- **事件系統** — 處理訊息、命令、通知、請求、元事件五大類事件
- **模組系統** — 透過獨立模組擴展功能，支援依賴管理和懶加載
...
`args=` 位置參數語法：`<count:int>` 必填、`[sides:int=6]` 可選（含默認值）。支援類型：
...
| `literal` | `<mode:literal=fast|slow>` | 陣列，僅接受列出的值；可選形式默認取首個 |
...
均支援；`throttle_key=` 與命令治理同一套鍵粒度（user / session / global），
...
不復用。可用 `Depends(get_db, use_cache=False)` 關閉單條依賴的緩存。
...
命令名支援**空格分隔**的多 token 形式，實現 `/admin add`、`/admin user ban` 這樣的子命令：
...
@command("restart", master=True, help="重新載入模組")
...
> **併發上限**：所有匹配 handler 的 Task 會**立即創建**，但透過一個信號量限制**同時在途執行數**，默認上限 **64**（`ErisPulse.framework.handler_max_concurrency`，支援熱更新）。超過上限的 Task 在信號量上排隊，等前面的完成後再進。事件洪峰時這就是你的「泄壓閥」。
>
...
詳情見 [i18n 推薦寫法](../../advanced/i18n.md#推薦寫法通過-i18nclass-聲明翻譯鍵-v270)。
...
所有使用者可見的字串欄位均支援 i18n，統一採用 `{"i18n": "key", "default": "文字"}` 格式，
...
框架的 `resolve_config_schema()` 會根據目前語言自動解析上述所有欄位的 i18n 鍵；
`get_config_schema()` 則原樣透傳 i18n 字典，由前端自行解析。

#### docstring 自动生成欄位描述（v2.8.0+）

未在 metadata 中聲明 `description` 的欄位，框架會自動從配置類 docstring 中
提取欄位說明作為兜底，支援兩種常見風格（可混用）：
...
優先級：**metadata description > docstring 欄位說明 > 空**。
i18n 字典形式的 description 不受影響（始終優先）。

#### 嵌套配置（v2.8.0+）

欄位類型為嵌套 dataclass 時，框架遞迴處理：schema 以 `"type": "table"` +
...
> ⚠️ `_schema_meta` 是類級元資料（非配置欄位）。若在 dataclass 類體內部聲明，
> 必須加 `ClassVar` 註解（`_schema_meta: ClassVar[dict] = {...}`），否則會被
> dataclass 視為普通欄位。框架對下劃線前綴欄位已做防禦性排除（不進入任何
> schema / 範本 / 預設值 / 驗證輸出），但仍建議規範聲明。
...
- **阻斷（stop）**：阻止向低优先级處理器傳播（`_propagation_stopped`）
...
`register_event_method` 和 `register_event_mixin` 支援傳入 `"*"` 作為平台名，註冊的方法會在**所有平台**的 Event 實例上都可用。適合 AI 對話、上下文管理等需要跨平台重用的功能模組。
...
OneBot12 規範允許實現自定義 `3xxxx` 的低三位。`34xxx` 語義為 **Platform Error**
