...
**訊息構建器**：ErisPulse 提供了 `MessageBuilder` 工具類別，用於方便地構建 OneBot12 訊息段列表，搭配 `Raw_ob12` 使用。詳見 [訊息構建器](#10-訊息構建器-messagebuilder) 章節。
...
7. **生命週期清理**：呼叫 `sdk.uninit()` 時，所有已註冊的生命週期事件處理器會被清理
