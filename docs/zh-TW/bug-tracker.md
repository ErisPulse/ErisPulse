...
> **如何閱讀 & 維護約定**
> - 每條 Bug 記錄包含問題描述、根因分析、影響版本範圍、修復方案等結構化欄位，建議升級前先檢索「影響版本」是否涵蓋當前使用的版本。
...
| **影響版本** | 受影響的版本區間，格式 `引入版本 - 修復版本`（包含兩端 dev 版本） |
| **修復版本** | 修復該 Bug 的具體版本號 |
| **修復內容** | 修復方案的簡要描述，包含關鍵程式碼變更點 |
...
| **關聯** | 相關 Issue / PR / Commit 鏈結 | 有外部討論記錄時補充 |
...
| 標識 | 級別 | 判定標準 | 典型表現 |
|------|------|---------|---------|
| 🔴 | 嚴重 | 導致進程崩潰、資料遺失/損壞、核心功能完全不可用、安全漏洞 | OOM Kill、訊息無法發送、模組無法載入、熱重載失敗 |
| 🟡 | 中等 | 功能異常但有迴避路徑、非核心功能失效、偶發問題 | 狀態判斷錯誤、重複觸發、快取過期、錯誤提示不準確 |
...
**嚴重性**: 🔴 嚴重 | 🟡 中等 | 🟢 輕微
**類型**: 配置系統 / 事件系統 / 适配器 / 路由 / 客戶端 / 儲存 / 加載系統 / CLI / 運行時
```
...
**問題**: `ModuleLoadStrategy` 提供了 `priority` 字段用於聲明模塊的初始化優先級，但加載策略的實現存在失誤，導致模塊未按預期的優先級順序初始化，實際按 `entry_points()` 的默認順序加載。當模塊間存在加載依賴時，無法通過 `priority` 確保正確的初始化先後關係。
...
**問題**: `ConfigManager` 的配置文件路徑默認為相對路徑 `"config/config.toml"`，在運行時依賴 `os.getcwd()` 解析。如果工作目錄在運行期間發生變化（例如通過 `os.chdir()`），配置文件的讀寫操作會指向錯誤的位置，導致配置丟失或讀取到舊數據。
...
1. 新增 `_validate_select_column()` 函數，僅攔截 SQL 注入危險字符（`;` `'` `"` `--` `/*` `*/` `\x00` 換行符）
...
2. `_watch_loop` 的 `except Exception` 不记录任何日志。
...
**问题**: 多個用戶報告使用 `config.setConfig(key, value)`（預設 `immediate=False`）後，自己的模塊配置未寫入 `config.toml`，而其它模塊的配置正常。設置 `immediate=True`（強制刷盤）可避開。表現為：運行期寫入的配置在下次重啟後丟失，啟動期模板生成的配置保留。
...
2. `_watch_loop` 整段持 `_lock`；真正外部修改時保留 `_dirty_keys`（merge 語義），下次 flush 與外部內容合併（臟鍵优先），不再 `clear()`
