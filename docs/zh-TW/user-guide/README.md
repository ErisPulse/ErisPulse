...
本指南幫助你安裝、配置和管理 ErisPulse 項目。

## 內容列表

| 文件 | 說明 |
|------|------|
| [安裝與配置](installation.md) | 系統需求、安裝方式（pip/uv/Docker）、驗證安裝 |
| [ErisPulse-App 手機/桌面客戶端](../ecosystem/app.md) | 官方客戶端：手機 / 桌面直接運行，原生介面管理 ErisPulse 實例 |
| [CLI 命令參考](cli-reference.md) | `epsdk` 命令列工具的完整使用說明 |
| [配置文件說明](configuration.md) | `config/config.toml` 各配置項的詳細說明 |
...
| `epsdk run main.py` | 運行專案（`--reload` 熱重載模式） |
| `epsdk list` | 列出已安裝的模組/適配器 |
| `epsdk upgrade <包名>` | 升級模組/適配器 |
| `epsdk doctor` | 診斷環境（Python/後端/配置/PyPI 連通性） |
