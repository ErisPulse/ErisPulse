...
框架启动与运行**不依赖数据库可达**——MySQL / PostgreSQL 连接失败不会导致框架崩溃或无法启动：

1. 建池瞬时失败自动指数退避重试（默认 3 次）
2. 重试耗尽 → 记录 WARNING（含原因与冷却时长），框架照常启动 / 继续运行，仅存储操作暂不可用
3. 冷却期（默认 30 秒）内后续存储操作**快速失败**（不阻塞、不拖慢其余功能）
4. 冷却结束自动重连试探——数据库恢复后存储随之恢复，无需重启

日志示例：`mysql 连接池创建最终失败（已重试 3 次），30 秒后自动重连；期间存储操作将快速失败，框架其余功能不受影响`
