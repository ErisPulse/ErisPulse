...
> **总纲**：[适配器标准化指南](standardization-guide.md) —— 标准化原则、标准地图、命名规则、差异处理模式与开发者 Checklist。新适配器/新能力请先读它。
...
6. [API 动作标准](api-action-spec.md) - OneBot12 标准 API 动作统一接口（用户/群组/频道/消息管理/文件含分片/元动作）
7. [适配器标准化指南（总纲）](standardization-guide.md) - 标准化原则/工作流/命名规则 + 交互组件标准（按钮键盘/交互回调事件）与各平台映射
