...
フレームワークは、`ErisPulse.*` の設定項目を環境変数で**上書き**できるようにサポートしています（Docker / コンテナ化 / CI 部署に適しており、`config.toml` を変更する必要がありません）。

命名規則：`ErisPulse.<section>.<key>` のドット区切りパスを、すべて大文字にし、`.` を `_` に置き換え、`ERISPULSE_` を接頭辞として付けます：
...
- **永続化されない**：上書きは実行時にのみ有効であり、`config.toml` には書き込まれません
...
- **優先度**：環境変数 > `config.toml` > 宣言されたデフォルト値（設定ファイルのホット更新後もこの優先度が維持されます）
- **型変換**：環境変数の値は、フィールドの注釈に応じて自動的に変換されます——`str` はそのまま、`int` / `float` / `bool`（`true` / `1` / `yes` / `on`）は自動変換、`list` / `dict` は JSON 解析を経ます；変換に失敗した場合は、その上書きを無視し（設定ファイルまたはデフォルト値に戻し）、警告を出力します
- **宣言1回、どこでも有効**：設定の読み取り、ホット更新、検証は同じパイプラインを使用します；設定パネルの Schema は `env` 名をマークし、`config.toml` のテンプレートのコメントには利用可能な環境変数が提示されます（テンプレートには環境変数の実際の値は書き込まれず、漏洩を防ぎます）
- **完全な互換性**：`env` を宣言していないフィールドの動作は変更されません；フレームワークの設定パイプラインを経ずに `ConfigClass` を直接インスタンス化した場合は、環境変数の影響を受けません

```bash
# Docker 部署の例：config.toml を変更せず、モジュールのキーを直接注入
