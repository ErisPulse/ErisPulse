...
フレームワークは、`ErisPulse.*` の設定項目を環境変数で**上書き**する機能をサポートしています（Docker / 容器化 / CI 部署に適しており、`config.toml` を変更する必要がありません）。
...
- **永続化しない**：上書きは実行中にのみ有効であり、`config.toml` には書き込まれません
- **ホットアップデートをサポート**：実行中に環境変数を変更した後、設定監視のリロードを組み合わせることで有効になります
...
- **優先度**：環境変数 > `config.toml` > 宣言されたデフォルト値（設定ファイルのホットアップデート後もこの優先度は維持されます）
- **型変換**：環境変数の値は、フィールドの注釈に従って自動的に変換されます——`str` はそのまま、`int` / `float` / `bool`（`true` / `1` / `yes` / `on`）は自動変換、`list` / `dict` は JSON 解析されます；変換に失敗した場合は、その上書きは無視され（設定ファイル / デフォルト値に回帰）、警告が表示されます
- **宣言1つでどこでも有効**：設定の読み取り、ホットアップデート、検証は同じパイプラインを使用します；設定パネルの Schema には `env` 名が表示され、`config.toml` のテンプレートのコメントには利用可能な環境変数が提示されます（テンプレートには環境変数の実際の値は書き込まれず、漏洩を防ぎます）
- **完全に互換性がある**：`env` を宣言していないフィールドの動作は変更されません；フレームワークの設定パイプラインを経由せずに `ConfigClass` を直接インスタンス化した場合、環境変数の影響を受けません
...
> **設定クラスとモデルフィールド、どちらを使うか？** 設定クラスは「モジュールの動作方法」を管理します（動作パラメータ、ホットアップデート）。
> ORM の `Field()` は「ユーザーがどのようなデータを生成したか」を管理します（データベーステーブル、クエリ）。両者は同一の制約語表と検証エンジンを共有します。対照表は [データモデル層 · 2つの宣言、どちらを使うか](../developer-guide/orm.md#2つの宣言、どちらを使うか) を参照してください。
...
1. [データモデル層（ORM）](orm.md) - 宣言的モデル、自動テーブル作成、追加・削除・更新・検索
...
1. [モジュールテスト（ErisPulse-Testing）](testing.md) - TestBot、イベントファクトリー、アサーション面
2. [モジュールトラブルシューティングガイド](module-troubleshooting.md) - モジュールがロードされない / イベントが反応しない / コマンドがトリガーされない問題の解決方法

### パブリッシングガイド

- [パブリッシュとモジュールストアガイド](publishing.md) - あなたの作品を PyPI および ErisPulse モジュールストアに公開する方法
