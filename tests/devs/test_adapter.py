...
class TestConfig:
    ...
    voice_file: str | None = "M5000017K7gL4WYnw2.mp3"
...
class TestRunner:
    ...
    def _check_response(self, response: Any) -> tuple[bool, dict | None]:
        """检查响应是否成功"""
        if response is None:
            return False, None
    ...
    async def run_test(self, test_num: int):
        ...
        try:
            ...
            print(f"  错误 - {type(e).__name__}: {e!s}")

        self.results.append(result)
        await asyncio.sleep(self.config.send_interval)

    async def _execute_test(self, test_num: int) -> Any | None:
        """执行具体的测试逻辑"""
